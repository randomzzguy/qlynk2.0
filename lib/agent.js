 ...
 */
export async function getAgentKnowledge(userId, supabaseClient) {
  const supabase = await getSupabaseClient(supabaseClient);
...
export function buildAgentSystemPrompt(config, documents = []) {
  ...
  parts.push(`You are ${config.agent_name || 'Your AI'}, the official digital twin and AI representative for this user.`);
 ...
 */
export async function createConversation(agentOwnerId, visitorId, visitorInfo = {}, supabaseClient) {
  const supabase = await getSupabaseClient(supabaseClient);
 ...
 */
export async function saveMessage(conversationId, role, content, supabaseClient) {
  const supabase = await getSupabaseClient(supabaseClient);
