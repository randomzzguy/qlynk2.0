...
async function requestKnowledgeEmbedding(input, embeddingSecret, timeoutMs) {
  ...
  if (payload?.model !== KNOWLEDGE_EMBEDDING_MODEL
      ...
      || payload.embeddings.length !== 1
      || !validateEmbedding(payload.embeddings[0])) {
...
export async function generateKnowledgeEmbeddings(inputs, options = {}) {
  ...
  const embeddings = [];
  for (let index = 0; index < normalizedInputs.length; index += MAX_CONCURRENT_EMBEDDINGS) {
    const batch = normalizedInputs.slice(index, index + MAX_CONCURRENT_EMBEDDINGS);
    embeddings.push(...await Promise.all(
      batch.map((input) => requestKnowledgeEmbedding(input, embeddingSecret, options.timeoutMs || 10_000)),
    ));
  }
  return embeddings;
}
