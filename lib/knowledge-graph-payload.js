...
export function sanitizeKnowledgeGraphExtraction(payload, sourceContent = '') {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)
      || !Array.isArray(payload.entities) || !Array.isArray(payload.claims)) {
    return validateKnowledgeGraphExtraction(payload, sourceContent);
  }

  let safePayload = { entities: [], claims: [] };
  for (const entity of payload.entities.slice(0, MAX_GRAPH_ITEMS)) {
    try {
      safePayload = validateKnowledgeGraphExtraction({
        entities: [...safePayload.entities, entity],
        claims: [],
      }, sourceContent);
    } catch {
      // An invalid or ungrounded entity and its dependent claims are discarded.
    }
  }

  for (const claim of payload.claims.slice(0, MAX_GRAPH_ITEMS)) {
    try {
      safePayload = validateKnowledgeGraphExtraction({
        entities: safePayload.entities,
        claims: [...safePayload.claims, claim],
      }, sourceContent);
    } catch {
      // Keep previously validated evidence and discard only the unsafe claim.
    }
  }

  return validateKnowledgeGraphExtraction(safePayload, sourceContent);
}
