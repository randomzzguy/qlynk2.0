import 'server-only';
import { validateKnowledgeGraphExtraction } from './knowledge-graph-payload.js';

const KNOWLEDGE_GRAPH_MODEL = 'llama-3.1-8b-instant';
const MAX_GRAPH_JOBS = 6;
...
function extractionPrompt(job) {
  return [
    ...
    'Use at most 16 entities and 16 claims. Return empty arrays when there are no safe atomic claims.',
...
async function extractGraph(job) {
  ...
  return validateKnowledgeGraphExtraction(payload, job.content);
}
...
export async function processPendingKnowledgeGraph({ supabase, ownerId = null, limit = MAX_GRAPH_JOBS } = {}) {
  ...
  const results = await Promise.all(jobs.map((job) => processGraphJob(supabase, job)));
