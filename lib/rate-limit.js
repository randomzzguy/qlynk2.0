...
async function checkIdentifierRateLimit(identifier, namespace, maxRequests, windowMs) {
  try {
...
export async function checkRateLimit(req, namespace, maxRequests, windowMs) {
  return checkIdentifierRateLimit(getRateLimitIdentifier(req), namespace, maxRequests, windowMs);
}

export async function checkRateLimitKey(identifier, namespace, maxRequests, windowMs) {
  if (typeof identifier !== 'string' || !identifier.trim()) {
    throw new Error('A rate-limit identifier is required');
  }
  return checkIdentifierRateLimit(identifier.trim(), namespace, maxRequests, windowMs);
}
...
export async function rateLimitResponseForKey(identifier, namespace, maxRequests, windowMs) {
  const result = await checkRateLimitKey(identifier, namespace, maxRequests, windowMs);
  if (result.allowed) return null;
  return createRateLimitResponse(result);
}
