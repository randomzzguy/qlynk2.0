 ...
 */
export async function isAgentLive(userId, providedSupabase = null) {
  try {
    const supabase = providedSupabase || createClientBrowser();
    ...
    if (subscription.tier === 'trial') {
      // If trial has ended, agent is always offline regardless of choice (as per user: "agent offline for 14 more days" if paused)
      if (trialEndsAt <= now) {
        return false;
      }
      return true;
    ...
    if (['creator', 'agency', 'pro', 'business'].includes(subscription.tier.toLowerCase())) {
